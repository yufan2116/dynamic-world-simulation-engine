...
export default function ActionBar({ value, onChange, onSubmit, disabled, placeholder, hintText }: Props) {
  ...
  return (
    <footer className="border border-fantasy-border/60 bg-black/60 backdrop-blur-lg p-3 rounded-lg">
      <p className="text-[10px] text-fantasy-muted mb-2">
        {hintText ?? "自由行动 · 在上方选择，或自行描述"}
      </p>
      <div className="flex gap-2">
        <input
          ...
          onKeyDown={handleKey}
          disabled={disabled}
          placeholder={placeholder ?? "例如：我假装喝醉，靠近守卫听他们谈话"}
